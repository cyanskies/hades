...
namespace ortho_terrain
{
	...
	const auto ortho_terrain_keyname = "ortho-terrain";
	...
	OrthoSave LoadVersion0(const YAML::Node &root)
	{
		OrthoSave output;
		//search for the ortho-terrain key
		auto ortho_node = GetNode(root, ortho_terrain_keyname);

		//get the tilesets
		auto tilesets_node = GetNode(ortho_node, tilesets_keyname);

		if (!tilesets_node.IsSequence())
			throw ortho_terrain::exception("Tileset node must be a sequence");

		for (const auto &t : tilesets_node)
		{
			if (!t.IsSequence())
				throw ortho_terrain::exception("Tileset element must be a sequence");

			if (t.size() != 2)
				throw ortho_terrain::exception("Tileset element must contain a name and starting tile_id value");

			auto id_node = t[0];
			if (!id_node || !id_node.IsScalar())
				throw ortho_terrain::exception("Tileset name not scalar");

			auto tileset_name = id_node.as<hades::types::string>();

			auto start_node = t[1];
			if (!start_node || !start_node.IsScalar())
				throw ortho_terrain::exception("Tileset starting tile_id not scalar");
			...
			if (start_tile < last_tile_id)
				throw ortho_terrain::exception("Tilesets must be organised with increasing tile id's.");
		...
		if (!width_node.IsScalar())
			throw ortho_terrain::exception("Width not scalar");

		output.map_width = width_node.as<tile_count_t>(0);

		if (output.map_width == 0)
			throw ortho_terrain::exception("Map Width invalid. Was wrong type, or value was 0");

		//get the tile list
		auto tiles_node = GetNode(ortho_node, tiles_keyname);

		if (!tiles_node.IsSequence())
			throw ortho_terrain::exception("Tiles node must conatain a sequence of unsigned integers");

		auto tile_count = tiles_node.size();

		if (tile_count % output.map_width != 0)
			throw ortho_terrain::exception("Not enough tiles to create a rectangular map.");
		...
		{
			if (!t.IsScalar())
				throw ortho_terrain::exception("Tile element must be a scalar unsigned integer");
	...
	OrthoSave Load(const YAML::Node &root)
	{
		try
		{
			...
			version_t version;
			if (version_node.IsDefined() && yaml_error(ortho_terrain_keyname, "n/a", version_keyname, "scalar", hades::data::UniqueId::Zero, version_node.IsScalar()))
				version = version_node.as<version_t>(max_supported_version + 1);

			if (version > max_supported_version)
				throw ortho_terrain::exception("Unable to read map, version string higher than supported level");
		...
		catch (YAML::Exception &e)
		{
			...
			throw ortho_terrain::exception(message.c_str());
		}
	}
	...
	YAML::Emitter &Save(const OrthoSave& obj, YAML::Emitter &target )
	{
		//add terrain header
		//ortho-terrain:
		target << YAML::Key << ortho_terrain_keyname;
