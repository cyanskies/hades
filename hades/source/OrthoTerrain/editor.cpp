...
{
	terrain GetEmptyTile(const tiles::resources::tile_settings *s)
	{
		const auto tileset = hades::data::Get<tiles::resources::tileset>(s->empty_tileset);
	...
	void terrain_editor::GenerateDrawPreview(const sf::RenderTarget& window, MousePos m)
	{
		assert(TileSettings);
		...
		if (Mode() == editor::EditMode::TILE)
		{
			...
			tile_editor::GenerateDrawPreview(window, m);
	}
	...
	void terrain_editor::SaveTiles(level &l) const
	{
	...
	void terrain_editor::LoadTiles(const level &l)
	{
	...
	draw_size_t terrain_editor::GetDrawSize() const
	{
		return _tileDrawSize;
	}

	void terrain_editor::_enterTileMode()
	{
	...
	void terrain_editor::_addTilesToUi()
	{
	...
	void terrain_editor::_addTiles(const std::vector<tile> &tiles)
	{
	...
	void terrain_editor::_setCurrentTile(tile t)
	{
